'use strict'
/** 截取控制台「发送固定弹幕」卡片，核对三列控件对齐。前置：主机 CDP 9224 已启动。 */
...
;(async () => {
  ...
  // 滚到固定弹幕卡片（截整视口，clip 在不同 DPI 下容易跑偏）
  await cdp(control, 1, 'Runtime.evaluate', {
    returnByValue: true,
    expression: `(() => {
      const card = document.getElementById('fixedText').closest('.card')
  ...
  fs.writeFileSync(path.join(__dirname, '..', '.e2e-fixed-card.png'), Buffer.from(shot.data, 'base64'))
  console.log('saved .e2e-fixed-card.png')
  process.exit(0)
